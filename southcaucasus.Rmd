...
```{r}
ggplot(turnover, aes(x = Year, y = `Imports + Exports (Thousands USD)`, color = Country)) +
  geom_line() +
  labs(title = "Total Trade with China (Imports + Exports), 2005-2018",
...
```{r}
ggplot(chnpos, aes(x = Year, y = Amount, color = Country)) +
  geom_point() +
  geom_line() +
